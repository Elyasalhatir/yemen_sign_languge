...
class MediapipePoseCalculator {
    constructor() {
        ...
        let setPoseBoneCalculator = () => {
            ...
            LeftArm.update = RightArm.update = function () {
                let spine = Spine;
                ...
                if (!spineXDirection || !spineZDirection || !armYDirection) return;

                // Calculate alignment with spine axes
                let alignX = Math.abs(spineXDirection.dot(armYDirection.clone().normalize()));
                let alignZ = Math.abs(spineZDirection.dot(armYDirection.clone().normalize()));

                // Use the axis that provides the most stable cross-product
                if (alignX < 0.7) {
                    // Arm is not aligned with X, safe to use X
                    ...
                    // Both aligned, use Y (up direction) as fallback
                    zDirection.crossVectors(spineYDirection, armYDirection);
                    zDirection.crossVectors(zDirection, armYDirection);
                }

                // Ensure Z is not zero
                if (zDirection.lengthSq() < 0.001) {
                    zDirection.set(0, 0, 1); // Fallback to global forward
                }
            ...
            LeftForeArm.update = RightForeArm.update = function () {
                // ForeArm inherits twist from parent Arm for consistency
                let parentArm = this.parent;
                let armZDirection = parentArm && parentArm.zDirection;
                let foreArmYDirection = this.yDirection;

                if (!armZDirection || !foreArmYDirection) return;

                // Project parent Z onto plane perpendicular to forearm Y
                zDirection.copy(armZDirection);
                let dot = zDirection.dot(foreArmYDirection);
                zDirection.addScaledVector(foreArmYDirection, -dot);

                if (zDirection.lengthSq() < 0.001) {
                    zDirection.copy(armZDirection); // Fallback
                }
        ...
        let setHandBoneCalculator = (isLeft) => {
            ...
            Wrist.update = function () {
                ...
                let vectorToPinky = lm_wrist && lm_wrist.getVectorToChild(1);  // Pinky MCP
                let vectorToIndex = lm_wrist && lm_wrist.getVectorToChild(4);  // Index MCP
                let vectorToMiddle = lm_wrist && lm_wrist.getVectorToChild(2); // Middle MCP

                if (vectorToPinky && vectorToIndex) {
                    // Palm normal (Z direction) = cross of fingers spread
                    zDirection.crossVectors(vectorToIndex, vectorToPinky);
                    zDirection.normalize();

                    // Y direction = pointing towards middle finger (or average of index/pinky)
                    if (vectorToMiddle) {
                        yDirection.copy(vectorToMiddle).negate();
                    } else {
                        yDirection.copy(vectorToPinky).add(vectorToIndex).negate();
                    }
                    yDirection.normalize();

                    // Flip Z for right hand to maintain consistent orientation
                    if (this.side === 'right') {
                        zDirection.negate();
                    }
